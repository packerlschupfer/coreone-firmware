...
usage: flash.sh <profile> [--yes]
  ...
  Restore to stock Prusa firmware:
    ...
    restore-h503-swd   SWD full restore (RDP gate). Or DFU+dump, no ST-Link (docs/h503_dfu_flashing.md).
...
EOF
  ...
  restore-h503-swd)
    cat <<'EOF'
== restore-h503-swd (SWD restore; DFU+dump is the no-ST-Link alternative) ==
