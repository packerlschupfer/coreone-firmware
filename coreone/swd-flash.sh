...
#
# Notes / safety:
#  - F427 uses connect-under-reset (NRST wired to probe) + `reset halt`, NOT plain `halt`.
#    ⚠ The old justification here -- "Klipper HIJACKS its SWD pins" -- was DISPROVED
#    2026-08-20: PA13/PA14 appear in no config, in no RESERVE_PINS_* list, and Klipper's
#    STM32F4 sources contain no SWD/DBGMCU disable. That claim most likely came from Prusa
#    STOCK firmware (which does lock the debug port) and was carried over by assumption.
#    Keep connect-under-reset for FLASHING regardless: you want the core halted at the reset
#    vector in a known state before erase/write. But it is NOT proven that a plain `halt`
#    fails on running Klipper -- untested since the premise fell.
#    ⚠ CONSEQUENCE, and the reason the printer resets whenever the ST-Link's USB is plugged
#    in AT ALL (no OpenOCD involved): NRST is wired to the probe, and the probe's reset pin
#    is undefined until its own firmware boots, so power-up glitches NRST into the F427.
#    Workaround: plug the probe's USB in FIRST, let it enumerate, THEN mate the SWD cable.
#    An attach-without-reset mode (`reset_config none` + plain `halt`) is plausible but
#    UNTESTED -- do not put it on the flash path. H503 = plain halt.
