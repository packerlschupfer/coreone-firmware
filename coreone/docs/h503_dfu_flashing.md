...
> **You may not need this ritual at all.** SWD via J9 (an ST-Link on SWDIO/SWCLK) needs no
> BOOT0, so it skips the entire strap/DFU dance below. This procedure exists to flash/restore
> the H503 **without a debugger**; if you own an ST-Link + TC2030 cable, use SWD instead.
...
- `J9 pad 1` = VTref = the 3V3 net, right next to TP1 — the strap target. ⚠ J9 is a
  TC2030 footprint: **six pads at 0.05" pitch with GND in the same cluster.** Clip onto
  pad 1 precisely — slipping one pad shorts 3V3 to GND. (TP24, on the U3 3V3-LDO output,
  is an alternative 3V3 source, but its physical placement on this board is unverified —
...
**Test points** (signal assignments read from the MK4-xBuddyExtension rev06 schematic —
electrically confirmed; **physical placement is UNVERIFIED**, and some may sit on the board
underside, so eyeball before probing):

| TP | net | | TP | net |
|---|---|---|---|---|
| TP1 | BOOT0 | | TP2 | nRESET |
| TP23 | V_5V | | TP3 | SWCLK |
| TP24 | V_3V3 | | TP4 | SWDIO |

`D16` = green 3V3-rail LED (via R81 620R). TP23=5V corroborates the bench-power note below;
TP2/TP3/TP4 break out nRESET/SWCLK/SWDIO as discrete points, handy for SWD without a TC2030 cable.
