...
| **usb** | self-contained klipper-puppy (@0x08000000), no bootloader — auto-boots | **one strap ever**: TP1-DFU (no ST-Link) *or* SWD to write it once, then **zero-touch USB updates** | `[mcu extension]` over USB — set-and-forget; updates jump to the H5 ROM-DFU over USB (`bootloader_request`), no strap or SWD |
| **rs485** | klipper-puppy behind the *stock* puppy bootloader (@0x08002000) | **RS-485, no ST-Link** onto the factory bootloader | **flash/verify only — not a runtime config.** No auto-boot; `FIRMWARE_RESTART` power-cycles the H503 back to its bootloader (drops PG2), so `[mcu extension]` can't persist across a reset |

All three present the enclosure (chamber fan/temp, RGBW/white LEDs, spool sensor) as a Klipper
`[mcu extension]` over the H503's own USB — autoboot and usb persistently, rs485 only while manually
started (see the Runtime column). **autoboot** is the best of both — one SWD flash of the
...
- **Permanent, no ST-Link (simplest layout):** `h503-usb` (noboot) — write it **once** via TP1-DFU
  (no ST-Link; see `h503_dfu_flashing.md`) or SWD, then it auto-boots and takes **zero-touch USB
  updates forever** (Klipper `bootloader_request` → H5 ROM-DFU — no strap, no SWD). Gated only on
  physically locating TP1 for that first flash.
- **Flash / verify with zero hardware — a test-drive, not a runtime config:** `h503-rs485` — no-tools
  `PUPPY_FLASH` + `PUPPY_START` onto the factory bootloader, fully reversible. But it **can't drive the
  enclosure day-to-day**: no auto-boot, and the `FIRMWARE_RESTART` needed to load `[mcu extension]`
  power-cycles the H503 back to its bootloader. Use it to prove the enclosure works, then move to
  usb or autoboot for permanent control.
...
> ⚠️ **Dump the factory H503 image BEFORE you write `usb` or `autoboot`.** Both land @0x08000000
> and overwrite the stock puppy bootloader; the dump is your only **no-ST-Link** route back to
> stock — a ~30 s `dfu-util` capture (`h503_dfu_flashing.md`, Phase 5). `rs485` leaves the
> bootloader intact, so it needs no dump.
...
- **H503, from rs485** — nothing to restore. The stock puppy bootloader (@0x08002000) is never
  overwritten, so just restore the F427 to stock and Buddy re-bootstraps the puppy app.
- **H503, from usb / autoboot** — these write @0x08000000 and replace the factory bootloader, so a
  restore is needed: **DFU + a prior dump** (no ST-Link — see `h503_dfu_flashing.md`), *or*
  `flash.sh restore-h503-swd` (SWD; RDP gate first).
