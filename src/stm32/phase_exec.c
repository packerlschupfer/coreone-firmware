...
#include "phase_shaper.h"      // struct phase_seg, the axis-space shaping evaluator
...
#define PE_MAX_RETIRE_PER_TICK 8 // Fix 4: cap segment retirements per prio-0 ISR tick. A host
                                // stall can dump a batch of already-EXPIRED segments; retiring
                                // ALL of them in one tick (was unbounded, up to ring depth 1023)
                                // turns a stall into a long ISR occupation -> AMPLIFIES the
                                // "Timer too close" it follows. Residue retires next tick; the
                                // clamped dry-coast in pe_update_angle covers the bail tick.
// Sentinel for phase_exec_osc's phase arg: keep the running accumulator and stage the new
// frequency/amplitude for the next zero crossing, instead of re-seeding the phase.
#define PE_OSC_KEEP_PHASE  0xffffffffu
#define PE_PRIME_DEPTH  16      // Fix 5: ring depth at which the margin low-water mark starts
                                // counting (gate the startup fill out of seg_min_depth). Was
                                // SEG_RING/2 = 512, which only ever primed because the SHAPED
                                // host feed bursts ~6 segs per move boundary and queues a whole
                                // flush horizon. MCU-side shaping makes the host emit RAW
                                // segments (~1/move, speed-INDEPENDENT, floored by min_seg_dt)
                                // -> depth never reaches 512 -> seg_primed never sets ->
                                // seg_min_depth/seg_min_rep stay pinned at SEG_RING and the
                                // diagnostic silently reports "infinite margin" exactly when
                                // the new feed path needs validating. A small ABSOLUTE depth
                                // primes under both feed models; 16 is still comfortably above
                                // the 1-2 segs present mid-first-fill.
...
// `struct phase_seg` now lives in phase_shaper.h -- the shaping evaluator and this file must
// agree on the layout bit-for-bit, and the golden test compiles that header too. Positions
// here are STEP-UNITS (same frame as stepper_get_position): 64 step-units = one electrical
// period, so electrical angle = (pos * PE_ANGLE_PER_STEP) & SINE_MASK. The reanchor flag is
// packed into duration's MSB -- read durations through PE_SEG_DUR(), never the raw field.
...
struct phase_exec {
    ...
    // resonance test (Prusa M958 analogue): hum the held angle in place (gantry doesn't
    // translate) -> phase_index += osc_amp*sin(osc_phase), osc_phase += osc_inc/tick.
    // Excitation is a PURE sinusoid at an exact frequency generated on the MCU, so a swept
    // measurement costs the host nothing per cycle -- unlike driving trapq accel pulses.
    uint8_t  osc_active;
    uint16_t osc_amp;
    // 32-bit phase accumulator, table index taken from the TOP 10 bits. Was uint16 with the
    // index from the LOW bits (osc_phase & SINE_MASK), which quantised the frequency to
    // rate/1024 -- 39 Hz steps at 40 kHz, useless for a swept-sine resonance measurement.
    // Now inc = freq/rate * 2^32, giving rate/2^32 ~= 1e-5 Hz resolution.
    uint32_t osc_inc;
    uint32_t osc_phase;
    // Staged frequency/amplitude change, applied at the next phase-accumulator WRAP.
    // A sweep must not step the commanded position between points: Prusa devotes a 27-line
    // comment (M958.cpp:407-433) to never stopping the motion, because a discontinuity drains
    // resonator energy by an amount depending on pause-duration mod resonator-period --
    // "non-deterministic energy loss caused by pauses could drastically alter measured
    // frequency spectrum". Applying at the wrap means sin(phase)==0, so BOTH the phase and the
    // amplitude change with the commanded offset at zero -- no position step either way.
    uint32_t osc_inc_next;
    uint16_t osc_amp_next;
    uint8_t  osc_pending;
    ...
    // MCU-side input shaping (Phase A). When shape=1 this motor stops reading its OWN
    // seg_ring as a motor trajectory and instead reads the two shared AXIS rings through
    // pe_axes[], convolves each with its own shaper, and projects:
    //     pos = cx*Xs + cy*Ys - pos_ref
    // For CoreXY that is cx=xs, cy=+ys on motor A ('+') and cy=-ys on B ('-'), with the
    // host folding its per-motor direction sign into the coefficients. Everything
    // downstream -- phase_offset, cogging, swap, lead_ticks, pack_xdirect, DMA -- is
    // untouched, because the ISR's OUTPUT is still a per-motor electrical angle.
    // ⚠ NO LONGER DEAD CODE. This defaulted 0 through Phase A/B, but `MCU_SHAPE` has
    // defaulted ON (auto) since 2026-08-01, so on any phase-stepped print the host binds
    // the axis rings and sets this -> the SHAPED path below is the live one and the legacy
    // motor-space path is the fallback (`MCU_SHAPE=0`), not the default.
    // Shipped public in v0.13.0-core-one.1.
    uint8_t  shape;
    struct pe_motor mot;      // projection coefficients + reanchor latch (phase_shaper.h).
                              // Kept over there so the projection/hold/reanchor arithmetic
                              // sits in the translation unit the golden test can reach.
...
// ---- MCU-side input shaping: the two shared AXIS evaluators -----------------
// pe_axes[0] = X, pe_axes[1] = Y. Each BORROWS an existing per-motor seg_ring (motor A's
// serves as X, motor B's as Y) rather than allocating its own: a second pair of 1024-entry
// rings would be +40 KB against a dynmem pool that does not have it to spare, and in shaped
// mode the per-motor rings are dead anyway. Bound by phase_exec_shape.
static struct pe_axis pe_axes[2];
static uint8_t pe_axes_bound;              // bitmask: which axes have a ring bound
// pe_axis holds head/tail as const volatile (it only reads them); retirement has to WRITE
// the owning executor's seg_tail, so keep a writable alias alongside.
static uint16_t *pe_axis_tailw[2];

// Pulse tables are staged and committed whole, so the ISR never reads a half-written table.
// CONTRACT: phase_exec_pulse is only accepted while every executor has shape=0 (the host
// ships the tables at engage, before enabling). Enforced in the command.
static struct pe_pulse pe_pulse_stage[2][PE_MAX_PULSE];

// Cycle-cost instrumentation for the shaped eval (plan A7). timer_read_time() is the DWT
// cycle counter on armcm, so this is a direct cycle count of both axis evaluations.
static uint16_t pe_shape_cyc_max, pe_shape_cyc_last;
static uint16_t pe_shape_hold;             // ticks held because the eval was empty/stale
// The two motors that project from the axis rings, indexed by the axis whose ring they
// donate. Kept as pointers so the reanchor edge can reach both without walking the oid table
// from a prio-0 ISR (oid_next is a linear scan -- far too expensive here).
static struct phase_exec *pe_shape_motor[2];
static uint8_t pe_reanchor_prev;           // last tick's PE_EVAL_REANCHOR (edge detect)
static uint8_t pe_shape_primed;            // the lookback window has been filled at least once
static uint16_t pe_shape_late_before;      // PE_EVAL_BEFORE fired AFTER priming = retention bug
static uint16_t pe_shape_reanchors;        // reanchor edges handled
...
{
    uint_fast8_t retired = 0;   // Fix 4: bound the prio-0 catch-up (PE_MAX_RETIRE_PER_TICK)
    while (1) {
        if (m->cur_valid) {
            ...
            m->cur_valid = 0;
            // Fix 4: a host stall can dump a batch of already-EXPIRED segments. Retiring the
            // whole backlog in one prio-0 tick (was unbounded, up to ring depth) turns a stall
            // into a long ISR occupation -> amplifies the "Timer too close" it follows. Cap the
            // drops/tick and return in the cur_valid==0 state: the CLAMPED dry-coast in
            // pe_update_angle (PE_MAX_DRY_SEC / PE_DRY_MAX_STEPS) covers THIS tick, and last_pos
            // has already advanced through the dropped endpoints, so the residue retires cheaply
            // over the next few ticks. (Bailing with cur_valid==1 would over-extrapolate the
            // expired quadratic -- pe_update_angle does NOT clamp epoch to duration.)
            if (++retired >= PE_MAX_RETIRE_PER_TICK)
                return;
        }
        ...
        uint8_t reanc = !!(m->cur_seg.duration & PE_SEG_REANCHOR);  // halt-resume flag
        m->cur_seg.duration &= PE_SEG_DUR_MASK;      // strip it -> duration logic unaffected
        if (reanc) {
...
// MCU-side input shaping: evaluate both AXIS rings at `tnow`, convolve each with its own
// shaper, and project onto THIS motor. Called once per tick (the ISR refreshes one motor per
// tick, round-robin), so the convolution is paid once per tick, not once per motor.
//
// DRY POLICY. pe_axis_eval is a pure evaluator: past the newest segment it FREEZES at that
// segment's endpoint, exactly as the host oracle does. That is deliberately not the legacy
// Fix-2 coast, and freezing is the safer of the two -- the whole reason the host now emits an
// explicit zero-velocity HOLD segment for idle spans (_trapq_flush_cb) is that coasting on a
// stale end-velocity flew the motor past 1/2 electrical period at a layer change and slipped
// permanently (the Y staircase). A coast, if it turns out to be wanted once the host actually
// feeds raw segments, must be applied HERE on the projected position -- never per-axis, or a
// simultaneous X and Y coast sums to 2x the PE_DRY_MAX_STEPS clamp. Phase C.
//
// EMPTY or LAGGED means the value is absent or stale (a saturated cursor walk evaluates an
// old segment = a wrong commanded angle), so hold last_pos rather than command it.
static void
pe_shaped_pos(struct phase_exec *m, uint32_t tnow, float *pos, float *vel)
{
    uint32_t t0 = timer_read_time();
    float xp = 0.f, xv = 0.f, yp = 0.f, yv = 0.f;
    uint8_t st = pe_axis_eval(&pe_axes[0], tnow, &xp, &xv);
    st |= pe_axis_eval(&pe_axes[1], tnow, &yp, &yv);
    // Edge-latch BEFORE stepping, so the motor being refreshed this tick anchors immediately.
    pe_shape_reanchors += pe_reanchor_edge(
        &pe_reanchor_prev, st,
        pe_shape_motor[0] ? &pe_shape_motor[0]->mot : 0,
        pe_shape_motor[1] ? &pe_shape_motor[1]->mot : 0);
    // Projection + hold policy + reanchor all live in phase_shaper.c, which the golden test
    // compiles and drives directly. m->phase_index is still last tick's held angle here
    // (pe_update_angle recomputes it after this returns), which is exactly what the reanchor
    // needs to tie the resumed position to.
    uint8_t pst = pe_motor_step(&m->mot, st, xp, xv, yp, yv, m->phase_index,
                                &m->last_pos, &m->phase_offset, pos, vel);
    if (pst & PE_PROJ_DRY) {
        m->seg_dry++;
        m->in_dry = 1;
    }
    if (pst & PE_PROJ_HELD)
        pe_shape_hold++;
    // Lookback priming. Before the ring spans the shaper's history window, PE_EVAL_BEFORE is
    // EXPECTED -- the impulses looking backwards fall before the first segment and the
    // evaluator holds that segment's start_pos, which at engage IS the seed position, so this
    // needs no special handling and produces no lurch. AFTER the window has filled once,
    // BEFORE means retained history was retired too early: a real bug. Counted separately so
    // the two cannot be confused during bring-up.
    //
    // Only judge priming on a USABLE evaluation. An EMPTY ring carries no BEFORE bit, so the
    // old `if (!(st & PE_EVAL_BEFORE))` set primed=1 on the very first idle tick and the flag
    // proved nothing -- measured on hardware as primed=1 with empty=12458 and not one segment
    // ever streamed. That also inflates late_before, because legitimate startup BEFOREs then
    // count as "late". LAGGED is excluded for the same reason: its value is stale.
    if (!(st & (PE_EVAL_EMPTY | PE_EVAL_LAGGED))) {
        if (!(st & PE_EVAL_BEFORE))
            pe_shape_primed = 1;
        else if (pe_shape_primed)
            pe_shape_late_before++;
    }
    // Retire: free every ring slot no cursor still needs. This is the ONLY thing advancing
    // seg_tail in shaped mode (pe_seg_advance is not called), and it is what bounds the
    // retained history to the shaper lookback -- measured 10.56 ms for EI@55.4 / MZV@43.4.
    if (pe_axis_tailw[0])
        *pe_axis_tailw[0] = pe_axis_retire(&pe_axes[0]);
    if (pe_axis_tailw[1])
        *pe_axis_tailw[1] = pe_axis_retire(&pe_axes[1]);
    // keep the ring-margin diagnostic alive on this path too: m's own ring IS an axis ring
    // here, so the same depth low-water mark applies (and PE_PRIME_DEPTH is what makes it
    // prime at all under the raw feed -- see its declaration).
    uint16_t depth = (m->seg_head - m->seg_tail) & (SEG_RING - 1);
    if (depth >= PE_PRIME_DEPTH)
        m->seg_primed = 1;
    if (m->seg_primed && depth < m->seg_min_depth)
        m->seg_min_depth = depth;
    // plan A7 cycle gate: timer_read_time() is the DWT cycle counter on armcm, so this is a
    // direct count for BOTH axis evaluations. Budget is 21000 cycles/tick @8kHz.
    uint16_t cyc = (uint16_t)(timer_read_time() - t0);
    pe_shape_cyc_last = cyc;
    if (cyc > pe_shape_cyc_max)
        pe_shape_cyc_max = cyc;
}
...
pe_update_angle(struct phase_exec *m, uint32_t now)
{
    ...
    if (m->analytic) {
        ...
        float pos, vel;
        if (m->shape) {
            // MCU-side shaping: position comes from the two shared AXIS rings, convolved
            // and projected, instead of from this motor's own segment cursor. Everything
            // after this if/else -- settle instrumentation, cogging blend, angle -- is
            // shared with the legacy path and needs no change.
            pe_shaped_pos(m, tnow, &pos, &vel);
            goto have_pos;
        }
...
have_pos:
    ...
    if (m->osc_active) {                       // hum in place around the held angle
        uint32_t prev_phase = m->osc_phase;
        m->osc_phase += m->osc_inc;
        if (m->osc_pending && m->osc_phase < prev_phase) {
            // unsigned wrap through 2^32 == phase 0 == sin 0 == zero commanded offset
            m->osc_inc = m->osc_inc_next;
            m->osc_amp = m->osc_amp_next;
            m->osc_pending = 0;
        }
        int32_t osc = ((int32_t)m->osc_amp
                       * sine_table[(m->osc_phase >> 22) & SINE_MASK]) >> 14;
...
command_config_phase_exec(uint32_t *args)
{
    ...
    e->osc_pending = 0;
    e->analytic = 0;
    e->shape = 0;             // MCU-side shaping OFF until the host binds + enables it
    e->mot.cx = e->mot.cy = e->mot.pos_ref = 0.f;
...
command_phase_exec_osc(uint32_t *args)
{
    ...
    e->osc_active = args[1];
    if (args[1]) {
        if (args[4] == PE_OSC_KEEP_PHASE) {
            // Sweep step: keep the accumulator running and STAGE the new frequency/amplitude
            // for the next wrap, so the excitation is continuous across the frequency change.
            e->osc_inc_next = args[3];
            e->osc_amp_next = args[2];
            e->osc_pending = 1;
        } else {
            // Start of a sweep: seed explicitly. CoreXY needs the two motors ANTIPHASE
            // (0 and 2^31) to excite Y, in phase to excite X -- without this the axis
            // selection silently excites the same 45-degree diagonal either way.
            e->osc_amp = args[2];
            e->osc_inc = args[3];
            e->osc_phase = args[4];
            e->osc_pending = 0;
...
command_phase_exec_stop(uint32_t *args)
{
    ...
    e->run_tmr = 0;
    e->active = 0;
    e->shape = 0;                          // a stopped run must not keep a stale axis binding
...
command_phase_exec_analytic(uint32_t *args)
{
    ...
    if (args[1]) {
        ...
        // the ring was just emptied -- park any axis cursor that borrows it, or the next
        // shaped eval walks from a stale index into slots the producer is about to reuse.
        for (uint8_t a = 0; a < 2; a++)
            if ((pe_axes_bound & (1 << a)) && pe_axes[a].ring == e->seg_ring)
                pe_axis_reset(&pe_axes[a], 0);
        if (e->dma) {
            e->dma->skips_same = 0;
...
// ---- MCU-side input shaping (Phase A) --------------------------------------
// Bind this executor's segment ring to an AXIS and set its CoreXY projection.
//   axis:    0 = this ring carries the X axis trajectory, 1 = Y, >=2 = bind nothing
//            (project only). Motor A donates X, motor B donates Y.
//   cx, cy:  projection coefficients, host folds its direction signs in ->
//            pos = cx*Xs + cy*Ys - pos_ref. CoreXY: cy=+ys on A ('+'), -ys on B ('-').
//   enable:  1 = read the axis rings instead of this motor's own segment cursor.
//
// Enabling is REFUSED unless both axis rings are bound: a half-bound pair would hand the
// ISR a null ring. Send from both motors (bind + coefficients), then the pulse tables,
// then re-send with enable=1.
void
command_phase_exec_shape(uint32_t *args)
{
    struct phase_exec *e = oid_lookup(args[0], command_config_phase_exec);
    uint8_t axis = args[1];
    if (axis < 2) {
        // pe_axis_init leaves any previously installed pulse table alone and only falls
        // back to the identity pulse if this axis was never configured, so a re-bind at
        // re-engage does not silently drop the shaper.
        pe_axis_init(&pe_axes[axis], e->seg_ring, SEG_RING - 1,
                     &e->seg_head, &e->seg_tail, pe_inv_clock);
        pe_axis_tailw[axis] = &e->seg_tail;
        pe_shape_motor[axis] = e;
        pe_axes_bound |= (uint8_t)(1 << axis);
    }
    e->mot.reanchor_pend = 0;
    e->mot.cx = u32_as_float(args[2]);
    e->mot.cy = u32_as_float(args[3]);
    e->mot.pos_ref = u32_as_float(args[4]);
    e->shape = (args[5] && (pe_axes_bound & 3) == 3) ? 1 : 0;
    if (e->shape) {                        // fresh diagnostics for the run being armed
        pe_shape_cyc_max = pe_shape_cyc_last = pe_shape_hold = 0;
        pe_shape_primed = pe_reanchor_prev = 0;
        pe_shape_late_before = pe_shape_reanchors = 0;
        for (uint8_t a = 0; a < 2; a++)
            pe_axes[a].n_before = pe_axes[a].n_past = pe_axes[a].n_empty
                = pe_axes[a].n_backscan = pe_axes[a].n_lagged = 0;
    }
}
DECL_COMMAND(command_phase_exec_shape,
             "phase_exec_shape oid=%c axis=%c cx=%u cy=%u pos_ref=%u enable=%c");

// Ship one shaper impulse. The table is staged and committed whole on the last index, so
// the ISR never reads a half-written table. a is an IEEE754 float bit-cast by the host; dt
// is a SIGNED offset in clock ticks (the offsets straddle zero -- see phase_shaper.h).
//
// Only accepted while shaping is off everywhere. Changing pulses under a live ISR would
// silently swap the resonance cancellation mid-move, so that is a shutdown, not a no-op:
// a silent wrong table is far worse than a halt.
void
command_phase_exec_pulse(uint32_t *args)
{
    uint8_t axis = args[0], idx = args[1], count = args[2];
    if (axis >= 2 || count == 0 || count > PE_MAX_PULSE || idx >= count)
        shutdown("phase_exec_pulse: bad axis/index");
    struct phase_exec *e;
    uint8_t oid = 0xff;
    while ((e = oid_next(&oid, command_config_phase_exec)))
        if (e->shape)
            shutdown("phase_exec_pulse while shaping is live");
    pe_pulse_stage[axis][idx].a = u32_as_float(args[3]);
    pe_pulse_stage[axis][idx].dt = (int32_t)args[4];
    if (idx + 1 == count)
        pe_axis_set_pulses(&pe_axes[axis], count, pe_pulse_stage[axis]);
}
DECL_COMMAND(command_phase_exec_pulse,
             "phase_exec_pulse axis=%c idx=%c count=%c a=%u dt=%i");

// Shaping diagnostics. cyc_* is the DWT cycle cost of BOTH axis evaluations in one tick
// (plan A7 gate: < ~15% of the 21000-cycle tick @8kHz). The n_* counters localize a feed
// problem: past = ring dry ahead, before = lookback window too short (retention bug),
// lagged = cursor walk saturated (value was held, not commanded), backscan = `now` stepped
// backwards. Separate from phase_exec_seg_state so that message's format is untouched.
// Stack high-water report. Scans the painted region from the BOTTOM of the stack upward;
// the first word that is no longer 0xC0FFEE00 is the deepest the stack has ever reached.
// Added for the hard-float switch: FP-in-ISR grows every exception frame from 32 to 104
// bytes (lazy stacking reserves the extended frame even when it defers the save), and
// CONFIG_STACK_SIZE is only 512 -- worst-case nesting arithmetic put us near the limit, so
// measure instead of guessing.
extern uint32_t _stack_start, _stack_end;
void
command_stack_usage(uint32_t *args)
{
    uint32_t *p = &_stack_start;
    uint32_t *end = &_stack_end;
    uint32_t total = (uint32_t)((char *)end - (char *)p);
    uint32_t free_bytes = 0;
    while (p < end && *p == 0xC0FFEE00) {
        free_bytes += 4;
        p++;
    }
    sendf("stack_usage_state oid=%c total=%u free=%u used=%u", args[0], total,
          free_bytes, total - free_bytes);
}
// oid is carried purely so each [phase_exec] instance gets its OWN response handler.
// Klipper keys handlers on (name, oid) and SILENTLY OVERWRITES on a duplicate, so an
// oid-less query registered by two objects leaves only the last one able to receive a
// reply -- the first one's send() then never resolves. The counters themselves are global.
DECL_COMMAND(command_stack_usage, "stack_usage oid=%c");

void
command_phase_exec_shape_query(uint32_t *args)
{
    sendf("phase_exec_shape_state oid=%c cyc_max=%hu cyc_last=%hu hold=%hu bound=%c"
          ...
          args[0], pe_shape_cyc_max, pe_shape_cyc_last, pe_shape_hold, pe_axes_bound,
          pe_shape_primed, pe_shape_late_before, pe_shape_reanchors,
          pe_axes[0].n_before, pe_axes[0].n_past, pe_axes[0].n_empty,
          pe_axes[0].n_backscan, pe_axes[0].n_lagged,
          pe_axes[1].n_before, pe_axes[1].n_past, pe_axes[1].n_empty,
          pe_axes[1].n_backscan, pe_axes[1].n_lagged);
}
DECL_COMMAND(command_phase_exec_shape_query, "phase_exec_shape_query oid=%c");
...
command_phase_exec_seg(uint32_t *args)
{
    ...
    sg->duration = args[2] | (args[6] ? PE_SEG_REANCHOR : 0u);
...
phase_exec_shutdown(void)
{
    ...
    pe_axes_bound = 0;               // bindings do not survive a shutdown
    pe_axis_tailw[0] = pe_axis_tailw[1] = 0;
    pe_shape_motor[0] = pe_shape_motor[1] = 0;
}
