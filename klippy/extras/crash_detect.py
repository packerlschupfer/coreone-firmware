...
class CrashDetect:
    def __init__(self, config):
        ...
        self._armed = False
        self._shadow = False            # set per-arm by CRASH_DETECT SHADOW=1
        self._last_shadow = {}          # last per-print shadow distribution (get_status)
    ...
    def _build_config(self):
        ...
        self.mcu.add_config_cmd(
            ...
            " sg_floor=0 gate_tstep=0 shadow=0" % (self._oid,),
            on_restart=True)
        ...
        # per-print shadow distribution (sent by the MCU on disarm of a shadow arm)
        self.mcu.register_serial_response(
            self._handle_shadow,
            "crash_detect_shadow oid=%c samples=%u min=%hu le0=%u le2=%u le5=%u"
    ...
    def disarm(self):
        ...
        self._arm_cmd.send([self._oid, 0, 0, 0, 0, 0, 0])
    ...
    def _cmd_CRASH_DETECT(self, gcmd):
        ...
        # SHADOW=1 arms telemetry-only mode: the MCU accumulates the gated-in sg_result
        # distribution and reports it on disarm INSTEAD of shutting down -- for tuning
        # sg_floor / crash_sgt from real PHASE_STEP=0 prints without risking a false halt.
        enable = gcmd.get_int('ENABLE', 1, minval=0, maxval=1)
        shadow = gcmd.get_int('SHADOW', 0, minval=0, maxval=1)
        ...
        # phase-stepping BEFORE arming crash detect, so this guard sees it. Also gates SHADOW.
        ...
        for _name, obj in self.printer.lookup_objects('crash_detect'):
            obj._shadow = bool(shadow)
            if enable:
                obj.arm()
            else:
                obj.disarm()
        gcmd.respond_info("Crash detection %s%s" % (
            "ON" if enable else "OFF",
            " (SHADOW telemetry -- no shutdown; reports the sg distribution on disarm)"
            if (enable and shadow) else ""))

    def _handle_shadow(self, params):
        # Per-print shadow distribution from the MCU (sent on disarm of a shadow arm).
        n = params['samples']
        mn = params['min']
        le = [params['le0'], params['le2'], params['le5'], params['le10'],
              params['le20'], params['le50'], params['le100']]
        wtrip = params['wtrip']
        self._last_shadow = {
            'samples': n, 'sg_min': (None if mn == 0xffff else mn),
            'le0': le[0], 'le2': le[1], 'le5': le[2], 'le10': le[3],
            'le20': le[4], 'le50': le[5], 'le100': le[6], 'would_trip': wtrip,
        }
        if not n:
            msg = ("crash shadow [%s]: 0 gated samples (no above-gate moves while armed)"
                   % self.name)
        else:
            msg = ("crash shadow [%s]: n=%d gated | sg_min=%s | <=0:%d <=2:%d <=5:%d "
                   "<=10:%d <=20:%d <=50:%d <=100:%d | would-trip(sgt=%d,%d-consec @floor%d)"
                   "=%d" % (self.name, n, ('-' if mn == 0xffff else mn),
                            le[0], le[1], le[2], le[3], le[4], le[5], le[6],
                            self.crash_sgt, self.sample_count, self.sg_floor, wtrip))
        logging.info(msg)
        try:
    ...
    def get_status(self, eventtime):
        st = {'armed': self._armed, 'shadow': self._shadow,
              'sg_floor': self.sg_floor, 'crash_sgt': self.crash_sgt}
        st.update(self._last_shadow)
        return st
