    ...
    def _cmd_TEMPERATURE_WAIT(self, gcmd):
        ...
        while not self.printer.is_shutdown():
            ...
            if deadline is not None and eventtime >= deadline:
                ...
                return
            toolhead.get_last_move_time()
            if i % 40 == 0:      # ~every 10s: clear progress so a long soak never reads as a hang
                goal = min_temp if min_temp != float('-inf') else max_temp
                gcmd.respond_info(
                    "TEMPERATURE_WAIT: %s %.1f -> %.0f (%.0fs elapsed%s)" % (
                        sensor_name, temp, goal, eventtime - start,
                        "" if deadline is None else ", %.0fs cap" % timeout))
            i += 1
            eventtime = self.reactor.pause(eventtime + self.poll)
