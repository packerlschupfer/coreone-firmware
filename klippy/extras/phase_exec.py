...
class PhaseExec:
    def __init__(self, config):
        ...
        # MCU-side shaping (Phase B). _mcu_shape selects the raw-axis emitter over the
        # host-shaped fitter; OFF by default until the Phase C hardware bring-up.
        self._mcu_shape = False
        self._flush_cb = self._trapq_flush_cb
        self._raw_last_xy = (0., 0.)
        self._raw_axis_oids = (None, None)
        self.shape_cmd = self.pulse_cmd = self.shape_query_cmd = None
        self.stack_query_cmd = None
        ...
        gcode.register_mux_command('PHASE_SWEEP', 'STEPPER',
                                   self.stepper_name, self.cmd_SWEEP,
                                   desc="Prusa M958-style MCU swept-sine resonance test")
        gcode.register_mux_command('PHASE_SHAPE_STATS', 'STEPPER',
                                   self.stepper_name, self.cmd_SHAPE_STATS,
                                   desc="Report MCU-side input-shaping diagnostics")
    ...
    def _build_config(self):
        ...
        self.osc_cmd = self.mcu.lookup_command(
            "phase_exec_osc oid=%c active=%c amp=%hu inc=%u phase=%u",
            cq=self.cmd_queue)
        ...
        self.seg_cmd = self.mcu.lookup_command(
            ...
            " reanchor=%c",
            cq=self.cmd_queue)
        # MCU-side input shaping (Phase B). Bind a motor's ring to an axis + its CoreXY
        # projection, and ship the shaper pulse tables. Both are no-ops on an MCU built
        # before this feature, so look them up defensively -- an older firmware simply
        # cannot run MCU_SHAPE=1 and ENGAGE says so instead of raising here.
        try:
            self.shape_cmd = self.mcu.lookup_command(
                "phase_exec_shape oid=%c axis=%c cx=%u cy=%u pos_ref=%u enable=%c",
                cq=self.cmd_queue)
            self.pulse_cmd = self.mcu.lookup_command(
                "phase_exec_pulse axis=%c idx=%c count=%c a=%u dt=%i",
                cq=self.cmd_queue)
            # oid on these two globals is NOT about per-motor state -- it is so each
            # [phase_exec] instance gets its own response handler. Klipper keys handlers on
            # (name, oid) and silently overwrites duplicates, so an oid-less query declared
            # by both stepper_x and stepper_y leaves only the last able to receive a reply.
            self.stack_query_cmd = self.mcu.lookup_query_command(
                "stack_usage oid=%c",
                "stack_usage_state oid=%c total=%u free=%u used=%u",
                oid=self.oid, cq=self.cmd_queue)
            self.shape_query_cmd = self.mcu.lookup_query_command(
                "phase_exec_shape_query oid=%c",
                # MUST match src/phase_exec.c's sendf EXACTLY -- klippy raises "Command
                # format mismatch" on any difference, and this string went stale once
                # already when primed/latebefore/reanchors were added MCU-side without
                # updating it here. The golden gate cannot catch that: it tests the
                # evaluator's arithmetic, not the host<->MCU message contract.
                "phase_exec_shape_state oid=%c cyc_max=%hu cyc_last=%hu"
                " hold=%hu bound=%c"
                " primed=%c latebefore=%hu reanchors=%hu"
                " x_before=%hu x_past=%hu x_empty=%hu x_back=%hu x_lag=%hu"
                " y_before=%hu y_past=%hu y_empty=%hu y_back=%hu y_lag=%hu",
                oid=self.oid, cq=self.cmd_queue)
        except Exception as e:
            # Log it. A bare swallow here means "no MCU-side shaping support in this
            # firmware" for ANY cause -- including a bug in these three lines -- which is
            # indistinguishable from an old MCU and wastes a bring-up cycle.
            logging.warning("phase_exec %s: MCU-shape command lookup failed (%s: %s)"
                            " -- MCU_SHAPE will be unavailable",
                            self.stepper_name, type(e).__name__, e)
            self.shape_cmd = self.pulse_cmd = self.shape_query_cmd = None
            self.stack_query_cmd = None
    ...
    def _eval_axis(self, moves, tau):
        # DO NOT DELETE -- golden reference for MCU-side shaping validation.
        # Together with _shaped_motor this is the numeric ORACLE that src/phase_shaper.c is
        # validated against (test/shaper_golden/). It looks near-dead in the hot path (the
        # forward-cursor _build_st_forward superseded it for streaming) -- it is not.
    ...
    def _shaped_motor(self, moves, T, xs, ys):
        # DO NOT DELETE -- golden reference for MCU-side shaping validation.
        # Only live caller is the rare PHGLITCH capture in _trapq_flush_cb, so this reads as
        # dead code. It is the numeric ORACLE the MCU-side convolution (src/phase_shaper.c) is
        # asserted against in test/shaper_golden/ -- the Phase A/D validation gate. Keep it
        # bit-faithful to kin_shaper.c; do not "simplify" it to match whatever the MCU does.
    ...
    def _report_shape_stats(self, gcmd):
        # Bring-up readout for MCU-side shaping. Every number here answers a specific
        # question that cannot be seen from the outside:
        #   cyc      -- the A7 cycle gate. Budget is 21000 cycles/tick at 8 kHz; the plan
        #               calls for < ~15% of that.
        #   primed   -- did the lookback window ever fill? 0 after a run = the ring never
        #               held the ~10.6 ms of history the shaper needs.
        #   lateBEF  -- PE_EVAL_BEFORE after priming = history retired too early. Must be 0.
        #               (Before priming it is expected and harmless, hence the split.)
        #   hold     -- ticks where the eval was EMPTY or LAGGED and the motor held instead
        #               of commanding a stale angle.
        #   past     -- ring dry ahead of the evaluated time; the eval froze at the endpoint.
        #   lag      -- forward cursor walk saturated (PE_ADVANCE_MAX).
        #   back     -- backward walk taken, i.e. `now` stepped backwards.
        #   reanch   -- halt-resume edges handled.
        if self.shape_query_cmd is None:
            return
        try:
            p = self.shape_query_cmd.send([self.oid])
        except Exception:
            return
        # Stack high-water. Reported alongside because the hard-float switch is what put it
        # at risk: FP-in-ISR grows every exception frame 32 -> 104 bytes against a 512-byte
        # CONFIG_STACK_SIZE, and the deepest nesting only happens under real motion -- which
        # is exactly when this command gets run.
        if self.stack_query_cmd is not None:
            try:
                sp = self.stack_query_cmd.send([self.oid])
                gcmd.respond_info(
                    "[phase_exec %s] MCU STACK used=%d / %d bytes (free=%d, %.0f%% used)"
                    % (self.stepper_name, sp['used'], sp['total'], sp['free'],
                       100.0 * sp['used'] / max(1, sp['total'])))
            except Exception as e:
                logging.warning("phase_exec: stack_usage query failed: %s", e)
        gcmd.respond_info(
            "[phase_exec %s] MCU-SHAPE cyc max=%d last=%d (budget %d/tick) | "
            "bound=0x%x primed=%d lateBEF=%d reanch=%d hold=%d | "
            "X past=%d bef=%d empty=%d back=%d lag=%d | "
            "Y past=%d bef=%d empty=%d back=%d lag=%d"
            % (self.stepper_name, p.get('cyc_max', -1), p.get('cyc_last', -1),
               self._interval or 0, p.get('bound', 0), p.get('primed', 0),
               p.get('latebefore', -1), p.get('reanchors', -1), p.get('hold', -1),
               p.get('x_past', -1), p.get('x_before', -1), p.get('x_empty', -1),
               p.get('x_back', -1), p.get('x_lag', -1),
               p.get('y_past', -1), p.get('y_before', -1), p.get('y_empty', -1),
               p.get('y_back', -1), p.get('y_lag', -1)))

    def cmd_SWEEP(self, gcmd):
        # PHASE_SWEEP -- Prusa M958-style swept-sine resonance measurement.
        #
        # The MCU hums the held electrical angle in place at an exact frequency
        # (phase_exec_osc), so the gantry does not translate and the host does NO per-cycle
        # work -- unlike driving trapq acceleration pulses, which is what [resonance_tester]
        # and belt_tune.py do and what this port had to fall back on while shaping was
        # host-side. One accelerometer capture spans the WHOLE sweep, which also avoids the
        # per-frequency CSV readout that starves the segment feed.
        #
        # Emits frequency markers to the log so the capture can be segmented afterwards.
        if not self._trapq_engaged:
            raise gcmd.error("PHASE_SWEEP: engage phase-stepping first")
        if self.osc_cmd is None:
            raise gcmd.error("PHASE_SWEEP: MCU has no phase_exec_osc")
        f0 = gcmd.get_float('FREQ_START', 20., minval=1.)
        f1 = gcmd.get_float('FREQ_END', 120., above=f0)
        step = gcmd.get_float('STEP', 2., above=0.)
        dwell = gcmd.get_float('DWELL', 0.25, above=0.02)
        # EXCITATION LEVEL. osc_amp is a PHASE amplitude, so a fixed amp gives an
        # acceleration that scales as f^2 -- which tilts the whole response curve toward high
        # frequencies and makes the envelope meaningless (measured: RMS rose 628->4918 across
        # 20-120 Hz purely from this). Prusa M958 and Klipper's [resonance_tester] instead
        # hold the EXCITATION constant, so do that here:
        #
        #   position amplitude   A[mm] = osc_amp / (SINE_LEN/PE_STEPS_PER_PERIOD) / steps_per_mm
        #                              = osc_amp / (16 * steps_per_mm)
        #   acceleration         a     = A * (2*pi*f)^2
        #   =>  osc_amp = a * 16 * steps_per_mm / (2*pi*f)^2
        #
        # ACCEL         constant acceleration across the sweep (Prusa M958 style)
        # ACCEL_PER_HZ  acceleration proportional to frequency (Klipper resonance_tester style)
        # AMP           legacy fixed phase amplitude; only for A/B against older captures
        fixed_amp = gcmd.get_int('AMP', 0, minval=0, maxval=512)
        accel = gcmd.get_float('ACCEL', 0., minval=0.)
        aph = gcmd.get_float('ACCEL_PER_HZ', 0., minval=0.)
        if not fixed_amp and not accel and not aph:
            accel = 1000.
        spm = self._steps_per_mm or 200.
        amp_scale = 16.0 * spm

        def amp_for(f):
            if fixed_amp:
                return fixed_amp, 0
            a = aph * f if aph else accel
            raw = a * amp_scale / ((2. * math.pi * f) ** 2)
            v = int(round(raw))
            clamped = 0
            if v > 512:
                v, clamped = 512, 1
        ...
        which = gcmd.get('AXIS', 'X').upper()
        # CoreXY: X moves both motors in phase, Y in antiphase. Driving ONE motor excites a
        # 45-degree diagonal; drive both to excite a true X or Y axis direction.
        # CoreXY: A and B in phase -> X direction; antiphase -> Y direction.
        targets = [(self, 0)]
        if slave is not None:
            targets = [(self, 0), (slave, 0 if which == 'X' else 0x80000000)]
        elif which == 'Y':
            raise gcmd.error("PHASE_SWEEP AXIS=Y needs the CoreXY pair (WITH=stepper_y)")
        n = int((f1 - f0) / step) + 1
        mode = ("AMP=%d (fixed phase)" % fixed_amp if fixed_amp
                else "ACCEL_PER_HZ=%.0f" % aph if aph else "ACCEL=%.0f mm/s^2" % accel)
        gcmd.respond_info("PHASE_SWEEP %s: %.1f-%.1fHz step %.1f dwell %.0fms %s (%d points,"
                          " phase-continuous)"
                          % (which, f0, f1, step, dwell * 1e3, mode, n))
        # osc() is an IMMEDIATE mcu command; toolhead.dwell() only reserves TOOLHEAD time.
        # Issuing all the frequency changes inside a dwell loop therefore fires them all off
        # at once and the MCU just sits at the last frequency -- the first version of this
        # did exactly that and produced a frequency/marker mapping that was pure fiction.
        # Block on wait_moves() after each dwell so the next frequency is only sent once the
        # previous one has actually been driven for `dwell` seconds of REAL time.
        marks = []
        nclamp = 0
        for i in range(n):
            f = f0 + i * step
            inc = self._osc_inc(f)
            amp, clamped = amp_for(f)
            nclamp += 1 if clamped else 0
            t_at = toolhead.get_last_move_time()
            for obj, ph in targets:
                # Seed the phase explicitly on the FIRST point only (this is what selects X
                # vs Y on CoreXY); afterwards keep the accumulator running so the excitation
                # never steps between frequency points. Prusa's equivalent invariant is
                # "never stop the movement during the sweep" -- a discontinuity drains
                # resonator energy by pause-duration mod resonator-period, which alternates
                # constructive/destructive between adjacent points and combs the spectrum.
                obj.osc(amp, inc, ph if i == 0 else self.OSC_KEEP_PHASE)
            marks.append((f, t_at, amp))
            toolhead.dwell(dwell)
            toolhead.wait_moves()
        for obj, _ph in targets:
            obj.osc_stop()
        toolhead.dwell(0.2)
        toolhead.wait_moves()
        # amp is logged PER POINT so the analysis can divide the measured response by the
        # excitation actually delivered -- which is what makes this a transfer function
        # rather than a raw response curve, and is the only way a clamped point is visible.
        logging.info("PHASE_SWEEP marks %s",
                     " ".join("%.1f:%.6f:%d" % (f, t, a) for f, t, a in marks))
        msg = ("PHASE_SWEEP done (%d points, %.1fs). Markers in klippy.log." % (n, n * dwell))
        if nclamp:
            msg += (" WARNING: %d point(s) hit the amp clamp -- excitation was NOT constant"
                    " there; lower ACCEL or narrow the range." % nclamp)
        gcmd.respond_info(msg)

    def cmd_SHAPE_STATS(self, gcmd):
        if not self._mcu_shape and self.shape_query_cmd is None:
            gcmd.respond_info("[phase_exec %s] no MCU-side shaping support in this firmware"
                              % (self.stepper_name,))
            return
        self._report_shape_stats(gcmd)

    def _ship_shaper_pulses(self):
        # Ship the live shaper pulse tables to the MCU. _load_shaper_pulses /
        # _init_shaper_pulses (the verbatim kin_shaper.c replica) are reused UNCHANGED --
        # the only difference from the host-shaped path is that the result gets SENT
        # instead of convolved locally. Offsets go as SIGNED clock ticks and straddle zero
        # (measured -10.56ms .. +7.58ms for EI@55.4 / MZV@43.4).
        freq = self.mcu.seconds_to_clock(1.0)
        for axis, (a_list, t_list) in ((0, self._px), (1, self._py)):
            n = len(a_list)
            for i in range(n):
                dt = int(round(t_list[i] * freq))
                self.pulse_cmd.send([axis, i, n, self._f2u(a_list[i]), dt])

    def _send_shape_binding(self, slave, x0, y0):
        # Bind each motor's segment ring to an AXIS and set its CoreXY projection.
        # Motor A donates its ring as the X axis ring, motor B as Y -- the ring reuse that
        # avoids allocating a second pair of 1024-entry rings.
        #
        # The projection reproduces the old host arithmetic exactly:
        #     posA = sa*(xs*X + ys*Y)*spm - refA
        #     posB = sb*(xs*X - ys*Y)*spm - refB
        # so cxA = sa*xs, cyA = sa*ys, cxB = sb*xs, cyB = -sb*ys, and pos_ref is that same
        # combination at the ENGAGE position -> pos = 0 at engage -> angle = MSCNT, the
        # seed convention the analytic path already uses.
        spm = self._steps_per_mm
        xs, ys = self._trapq_xsign, self._trapq_ysign
        sa, sb = self._trapq_sign, self._trapq_sign_b
        cxA, cyA = sa * xs, sa * ys
        cxB, cyB = sb * xs, -sb * ys
        refA = (cxA * x0 + cyA * y0) * spm
        refB = (cxB * x0 + cyB * y0) * spm
        # TWO passes. The MCU refuses `enable` until BOTH axis rings are bound, so a single
        # pass would leave the master at shape=0 (only axis 0 bound when its command lands)
        # while the slave came up enabled -- a half-shaped pair. Bind both, then enable both.
        for en in (0, 1):
            self.shape_cmd.send([self.oid, 0, self._f2u(cxA), self._f2u(cyA),
                                 self._f2u(refA), en])
            self.shape_cmd.send([slave.oid, 1, self._f2u(cxB), self._f2u(cyB),
                                 self._f2u(refB), en])

    @staticmethod
    def _raw_axis_segments(moves, t0, t1, spm, clock_of, last_xy, reanchor=False):
        # RAW per-AXIS segment emitter for MCU-side shaping (Phase B).
        #
        # This is what replaces the whole shaped-fitting machinery below. Within a trapq
        # move BOTH axes are already exactly constant-accel --
        #     x(t) = start_x + x_r*(start_v*t + accel/2*t^2)
        # -- so one move becomes exactly ONE X segment and ONE Y segment, with no
        # evaluation, no breakpoint set, no merge search, no vel-fit, and therefore no
        # fit error to chain or slew away. Everything that existed to manage that error
        # (DT_MAX grid, pulse-shifted breakpoints, velocity zero-crossing insertion,
        # HAMAX clamp, the chain + slew loop, _maxdiv/_maxsegerr) is simply not needed
        # here: start_pos is the true trajectory position every time.
        #
        # PURE by construction -- no self, no MCU I/O, clock conversion injected. That is
        # what lets test/shaper_golden/ drive the real emitter through the real MCU
        # evaluator and compare against the oracle.
        #
        # Returns (steps, end_pt, end_xy). Each step is one time slice covering BOTH axes:
        #   (start_clock, duration, end_pt, (end_x, end_y), (xpos,xv,xha), (ypos,yv,yha), ra)
        # Clocks TELESCOPE: each step's start_clock is the previous step's exact end clock,
        # so the ring is gapless (the invariant pe_axis_eval's forward cursor relies on).
        steps = []
        lx, ly = last_xy
        t = t0
        tc = clock_of(t0)
        ra = 1 if reanchor else 0
        for m in moves:
            if m.move_t <= 1e-9:          # zero-duration (arc-vertex shadow) -> no motion
                continue
            if m.print_time < t - 1e-9:   # already emitted in an earlier window
                continue
            if m.print_time >= t1 - 1e-9:
                break
            if m.print_time > t + 1e-9:
                # dwell/gap: emit an explicit zero-velocity HOLD. pe_axis_eval would freeze
                # correctly across a literal hole anyway, but keeping the ring contiguous
                # is cheap and it is the documented precondition of the evaluator.
                gc = clock_of(m.print_time)
                if gc > tc:
                    steps.append((tc, gc - tc, m.print_time, (lx, ly),
                                  (lx * spm, 0.0, 0.0), (ly * spm, 0.0, 0.0), ra))
                    ra = 0
                    tc = gc
                t = m.print_time
            ec = clock_of(m.print_time + m.move_t)
            if ec <= tc:
                continue
            dist = m.start_v * m.move_t + 0.5 * m.accel * m.move_t * m.move_t
            ex = m.start_x + m.x_r * dist
            ey = m.start_y + m.y_r * dist
            t = m.print_time + m.move_t
            steps.append((tc, ec - tc, t, (ex, ey),
                          (m.start_x * spm, m.x_r * m.start_v * spm,
                           0.5 * m.x_r * m.accel * spm),
                          (m.start_y * spm, m.y_r * m.start_v * spm,
                           0.5 * m.y_r * m.accel * spm), ra))
            ra = 0
            lx, ly = ex, ey
            tc = ec
        if t < t1 - 1e-9:
            # trailing idle: hold to the flush horizon so the MCU's shaper LOOKAHEAD window
            # (measured 7.58ms for MZV@43.4 / EI@55.4) stays fed instead of reading past the
            # ring head. Moves before flush_time are final, so nothing can appear here later.
            ec = clock_of(t1)
            if ec > tc:
                steps.append((tc, ec - tc, t1, (lx, ly),
                              (lx * spm, 0.0, 0.0), (ly * spm, 0.0, 0.0), ra))
            t = t1
        return steps, t, (lx, ly)

    def _trapq_flush_raw_cb(self, flush_time, step_gen_time):
        # Phase B flush path: forward RAW axis trapezoids and let the MCU shape them.
        #
        # NOTE the horizon is flush_time, NOT flush_time - _pulse_margin. The margin existed
        # because the host evaluated shaped samples at T+t_i and needed those moves present.
        # It no longer evaluates anything, so the lag disappears -- a small latency win on
        # top of the segment-count win.
        ...
        GAP_MAX = 1.0
        if t1 - t0 > GAP_MAX:
            # long idle while engaged (PAUSE / M600 / TEMPERATURE_WAIT): skip the gap rather
            # than emitting a >12.8s hold, whose duration would collide with the reanchor
            # flag packed in duration's MSB.
        ...
        cnt = self._ffi_lib.trapq_extract_old(self._trapq, data, cap, t0, t1)
        if cnt >= cap:
            logging.warning("phase_exec %s: trapq extract cap HIT (%d); raise _extract_cap",
                            self.stepper_name, cnt)
        ...
        while infl and infl[0] < now_clock:
            infl.pop(0)
        steps, end_pt, end_xy = self._raw_axis_segments(
            moves, t0, t1, self._steps_per_mm, self.mcu.print_time_to_clock,
            self._raw_last_xy, self._reanchor_next)
        xo, yo = self._raw_axis_oids
        sent = 0
        for (sc, dur, ept, exy, xt, yt, ra) in steps:
            if len(infl) >= self._max_inflight:
                break                      # backpressure: resume here next flush
            m32 = sc & 0xffffffff
            self.seg_cmd.send([xo, m32, dur, self._f2u(xt[0]),
                               self._f2u(xt[1]), self._f2u(xt[2]), ra])
            self.seg_cmd.send([yo, m32, dur, self._f2u(yt[0]),
                               self._f2u(yt[1]), self._f2u(yt[2]), ra])
            infl.append(sc + dur)
            self._trapq_last_pt = ept
            self._raw_last_xy = exy
            self._trapq_nseg += 1
            sent += 1
            if ra:
                self._reanchor_next = False
        if sent == len(steps):
            self._trapq_last_pt = end_pt
            self._raw_last_xy = end_xy
        if self._trapq_debug and steps:
            logging.info("PHRAW steps=%d sent=%d t=[%.4f,%.4f] xy=(%.3f,%.3f) infl=%d",
                         len(steps), sent, t0, self._trapq_last_pt,
                         self._raw_last_xy[0], self._raw_last_xy[1], len(infl))
    ...
    def cmd_TRAPQ_ENGAGE(self, gcmd):
        ...
        # MCU_SHAPE=1 (Phase B): stop convolving on the host -- forward RAW axis trapezoids
        # and let the F427 do the shaping. Defaults OFF: the host-shaped path stays the
        # proven default until the Phase C hardware bring-up validates this one. Flipping the
        # default (and then deleting the shaped-fitting machinery) is Phase C/D work.
        # MCU_SHAPE now DEFAULTS ON (bring-up steps 0-5 passed on hardware 2026-08-01:
        # shaped motion exact, 5.5x ringing suppression matching the host path within 6%,
        # 1250 cycles = 30% of the 40kHz tick, stack 29%). MCU_SHAPE=0 forces the legacy
        # host-shaped fitter, which stays in the tree as the fallback.
        #
        # -1 = auto: on wherever it actually applies. This must NOT hard-fail the cases it
        # cannot serve -- IS=0 (the unshaped control used by resonance/cogging work), a
        # single non-CoreXY motor, or an MCU built without src/phase_shaper.c. Those fall
        # back silently; only an EXPLICIT MCU_SHAPE=1 raises, because then the user asked.
        _req = gcmd.get_int('MCU_SHAPE', -1, minval=-1, maxval=1)
        _explicit = _req >= 0
        _can = (corexy and self._shape
                and self.shape_cmd is not None and self.pulse_cmd is not None)
        mcu_shape = (bool(_req) and _can) if _explicit else _can
        if _explicit and _req and not _can:
            if not corexy:
                raise gcmd.error("MCU_SHAPE requires a CoreXY pair (both axis rings)")
            if self.shape_cmd is None or self.pulse_cmd is None:
                raise gcmd.error("MCU_SHAPE: this firmware has no phase_exec_shape/"
                                 "phase_exec_pulse (rebuild with src/phase_shaper.c)")
            raise gcmd.error("MCU_SHAPE with IS=0 / no [input_shaper]: nothing to "
                             "shape -- use the plain analytic path instead")
        self._mcu_shape = mcu_shape
        ...
        # MCU-shape state: the raw emitter tracks the axis position itself (there is no
        # per-motor pos_ref to subtract host-side any more -- the MCU does that).
        th_pos = self.printer.lookup_object('toolhead').get_position()
        self._raw_last_xy = (th_pos[0], th_pos[1])
        self._raw_axis_oids = (self.oid, slave.oid) if corexy else (self.oid, self.oid)
        ...
        if mcu_shape:
            # ORDER MATTERS. analytic ON first (it clears the rings and parks any axis
            # cursor that borrows them), THEN the pulse tables -- the MCU shuts down on a
            # pulse sent while shaping is live -- THEN bind + enable. Enabling before
            # start_cmd is deliberate: the rings are empty, so the first ticks evaluate
            # EMPTY and hold at last_pos = 0, i.e. angle = phase_offset = MSCNT. That is
            # the same seed convention the analytic path already engages with.
            self._ship_shaper_pulses()
            self._send_shape_binding(slave, th_pos[0], th_pos[1])
        self._send_corrections()
        if corexy:
        ...
        self._flush_cb = (self._trapq_flush_raw_cb if mcu_shape
                          else self._trapq_flush_cb)
        ...
        if self._shape:
            ...
            shp = " IS=off"
        shp += " SHAPE=%s" % ("MCU" if mcu_shape else "host")
    ...
    def cmd_TRAPQ_DISENGAGE(self, gcmd):
        ...
        slave = getattr(self, '_group_slave', None)
        if self._mcu_shape:
            # Report the shaping diagnostics BEFORE tearing down (phase_exec_stop clears
            # `shape`, and the counters are what the bring-up is actually reading).
            self._report_shape_stats(gcmd)
            self._mcu_shape = False
            self._flush_cb = self._trapq_flush_cb   # next engage re-selects explicitly
    ...
    def _osc_inc(self, freq_hz):
        # 32-bit phase accumulator on the MCU, table index from the TOP 10 bits, so one full
        # electrical-angle period is 2^32 of accumulator. inc = freq/rate * 2^32.
        # (Was a 16-bit accumulator indexed from the LOW bits, which quantised frequency to
        # rate/1024 = 39 Hz steps at 40 kHz -- unusable for a sweep.)
        rate = self._engage_rate or 40000.
        inc = int(round(freq_hz / float(rate) * 4294967296.0))
        return max(1, min(inc, 0xffffffff))

    OSC_KEEP_PHASE = 0xffffffff   # must match PE_OSC_KEEP_PHASE in src/phase_exec.c

    def osc(self, amp, inc, phase=0):
        # hum the held angle in place at a fixed freq (inc from _osc_inc()).
        # phase: 0 or 2**31 (180 deg) -- CoreXY antiphase selects Y over X.
        # phase=OSC_KEEP_PHASE -- keep the running accumulator and stage the new
        # frequency/amplitude for the next zero crossing (continuous sweep step).
        if self.osc_cmd is not None:
            self.osc_cmd.send([self.oid, 1, int(amp), int(inc), int(phase) & 0xffffffff])

    def osc_stop(self):
        if self.osc_cmd is not None:
            self.osc_cmd.send([self.oid, 0, 0, 0, 0])
