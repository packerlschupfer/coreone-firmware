...
- **Host:** Raspberry Pi **4** (2 GB+) minimum for a complete machine — validated (10+ prints).
  **Pi 5 recommended.** Klipper runs kinematics and step generation on the *host* CPU
  (single-threaded per MCU) and must deliver step timing ahead of real time; the host also does
  ongoing loadcell force-FIR (clog detection) and filament-sensor processing during prints.

  **Pi 3: usable for an F427-only build, not for the full printer.** Two separate limits, and the
  one that actually stops you is *not* CPU:

  - **USB is the hard blocker.** The Pi 3's USB is `dwc_otg`, serviced by an **FIQ** — a
    fast-interrupt path that structurally bypasses `smp_affinity`, so **`isolcpus` does not give
    Klipper an interrupt-quiet core on a Pi 3** (measured: ~2000 IRQ/s landing on the supposedly
    isolated core; `dwc_otg.speed=1` cuts it ~8x, both our MCUs being full-speed devices anyway).
    With **both** MCUs attached that load stalls PID1 past the 60 s systemd hardware watchdog and
    the board hard-resets, in a loop. A Pi 4 is immune — xHCI behind PCIe-MSI honours affinity
    (measured **zero** USB interrupts on Klipper's core). So on a Pi 3 you give up the enclosure
    extension board: no chamber fan or temperature, RGBW lighting or spool sensor.
  - **CPU is no longer the constraint it once was.** Input shaping now runs **on the F427**, not
    the host: the host streams raw un-shaped *axis* segments and the MCU convolves per tick
    (Prusa parity — Prusa shapes on the Buddy MCU too). Measured on a dense-motion file,
    F427-only, a Pi 3 **matches** a Pi 4 on feed margin — 90,608 segments on both, 1 vs 2
    trajectory-starvation events, zero refills, no "Timer too close" — at 6.8 % of one core
    against the Pi 4's 3.0 %.

  Forcing shaping back onto the host costs ~248x the starvation events on a Pi 3 — and ~56x on a
  Pi 4, so **host-side shaping is expensive on any host; a slow host compounds it rather than
  causing it.** (The Pi 4 figures were taken with both MCUs and a camera attached where the Pi 3
  ran one MCU and none, so the comparison is conservative toward the Pi 3, not flattering to it.)
